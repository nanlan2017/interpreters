(module data-structures (lib "eopl.ss" "eopl")
  ...
  (define-datatype Thread Thread?
    ($a-thread
     (id integer?)
     (p procedure?)
     (may-mut (maybe Mutex?))   ; 为每个thread可关联一个mutex，便于: No. --> %list-of-threads --> 【target thread --> target mutex】--> kill from wait-queue
     ))  

  (define (thread->id th)
    (cases Thread th
      ($a-thread (id p maybe-mut)
                 id)))
  ...
  (define (thread->maybe-mutex th)
    (cases Thread th
      ($a-thread (id p may-mut)
                 may-mut)))

  ; run-thread 
  (define (run-thread th-c)
    (eopl:printf "——————————————> run-thread: Start running thread: #~s ~n" (thread->id th-c))
    ((thread->proc th-c)))
  ;--------------------------------------
  ; “锁” 
